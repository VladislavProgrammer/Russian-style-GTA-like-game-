...
public class RCC_Camera : RCC_Core {
    ...
    private void TPS() {
        ...
        if (TPSLockX)
            xAngle = Mathf.SmoothDampAngle(transform.eulerAngles.x, cameraTarget.playerVehicle.transform.eulerAngles.x + orbitY, ref xVelocity, 1f - rotDamp);
        ...
        if (TPSLockY) {
            ...
            if (orbitWithRotationDamping)
                yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, (cameraTarget.playerVehicle.transform.eulerAngles.y + orbitX + dir), ref yVelocity, 1f - rotDamp);
            else
                yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, (cameraTarget.playerVehicle.transform.eulerAngles.y + orbitX + dir), ref yVelocity, .025f);

        } else {

            //  If orbit for TPS is enabled, process orbit Y. Otherwise process Y of the player vehicle only.
            if (orbitWithRotationDamping)
                yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, orbitX, ref yVelocity, 1f - rotDamp);
            else
                yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, orbitX, ref yVelocity, .025f);
