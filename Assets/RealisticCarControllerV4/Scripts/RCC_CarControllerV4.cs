...
public class RCC_CarControllerV4 : RCC_Core {
    ...
    public bool canControl = false;              // Enables / Disables controlling the vehicle. If enabled, vehicle can receive all inputs from the InputManager.
    ...
    public AnimationCurve steerAngleCurve = new AnimationCurve(new Keyframe(0f, 40f, 0f, -.3f), new Keyframe(120f, 10f, -.115f, -.1f), new Keyframe(200f, 7f));   //	Steering angle limiter curve based on speed.
    ...
    private float oldEngineTorque = -1f;             // Old engine torque used for recreating the engine curve.
    private float oldMaxTorqueAtRPM = -1f;           // Old max torque used for recreating the engine curve.
    private float oldMinEngineRPM = -1f;             // Old min RPM used for recreating the engine curve.
    private float oldMaxEngineRPM = -1f;             // Old max RPM used for recreating the engine curve.
    #endregion
    ...
    public GameObject contactSparkle { get { return Settings.contactParticles; } }     // Contact Particles for collisions. It must be Particle System.
    public GameObject scratchSparkle { get { return Settings.scratchParticles; } }     // Scratch Particles for collisions. It must be Particle System.
    ...
    public void DeactiveEngine()
    {
        canControl = false;
        engineSoundIdle.Pause();
    }
