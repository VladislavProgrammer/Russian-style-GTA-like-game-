...
public class RCC_CameraEditor : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (RCCCam.useFixedCameraMode) {
            ...
            if (!FindObjectOfType<RCC_FixedCamera>()) {
                ...
                if (GUILayout.Button("Create Fixed Camera System")) {
                    ...
                    Undo.RegisterCreatedObjectUndo(fixedCamera, "Add RCC_FixedCamera");
        ...
        if (RCCCam.useCinematicCameraMode) {
            ...
            if (!FindObjectOfType<RCC_CinematicCamera>()) {
                ...
                if (GUILayout.Button("Create Cinematic Camera System")) {
                    ...
                    GameObject cinematicCamera = Instantiate(RCC_Settings.Instance.cinematicCamera, Vector3.zero, Quaternion.identity);

                    // Register the creation of the object for undo/redo functionality.
                    Undo.RegisterCreatedObjectUndo(cinematicCamera, "Add RCC_CinematicCamera");
        ...
        EditorGUILayout.PropertyField(serializedObject.FindProperty("orbitWithRotationDamping"), new GUIContent("Orbit With Rotation Damping", "Rotates the camera related to damping factor."), false);
...
#if RCC_URP
        if (!EditorUtility.IsPersistent(RCCCam) && RCC_PipelineChecker.IsURP())
            RCCCam.CheckURP_PP();
#endif
